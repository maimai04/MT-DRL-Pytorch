...
def train_PPO(env_train, save_name, trained_dir, timesteps=50000):
    ...
    logging.info("Training time (PPO): "+str((end_ppo - start_ppo) / 60)+" minutes.")
...
def DRL_trading(df,
    ...
    for i in range(len(trade_data.index.unique())):
        ...
        if i == (len(trade_data.index.unique()) - 2):
            ...
            last_state_flattened, last_state = env_trade.render()
    ...
    logging.info("Trading time: "+str((end - start) / 60)+" minutes")
    return last_state
...
def DRL_validation(model, test_data, test_env, test_obs) -> None:
    ...
    logging.info("Validation time: "+str((end - start) / 60)+" minutes")
...
def run_model(df,
    ...
    logging.info("=================Starting {}-only Strategy================".format(settings.STRATEGY_MODE))
    ...
    for i in range(settings.REBALANCE_WINDOW + settings.VALIDATION_WINDOW,  # from (63+63 = 128 trading days)
        ...
        logging.info("PPO Sharpe Ratio: "+str(sharpe_ppo))
        ...
        logging.info(f"---PPO Trading from: {trading_beginning} to {trading_ending}, (i={i}).======")
